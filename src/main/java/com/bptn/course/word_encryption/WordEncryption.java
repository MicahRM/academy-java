package com.bptn.course.word_encryption;
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < word.length(); i++ ) {   //for loop that iterates the length of the word.
			if(word.charAt(i) == 'z') {             //if-else statement to handle 'z'
				...
				System.out.print('A');              //prints the encrypted 'A' if 'Z' is encountered in string traversal
			} else if(word.charAt(i) >= 'a' && word.charAt(i) < 'z' || word.charAt(i) >= 'A' && word.charAt(i) < 'Z'){    //if-else statement to determine alphabetical characters in the word based on the ASCII chart (excluding 'z' and 'Z')  
				System.out.print((char) (word.charAt(i) + 1));                                                            //prints the the encrypted char after the ASCII value has been increased by 1
