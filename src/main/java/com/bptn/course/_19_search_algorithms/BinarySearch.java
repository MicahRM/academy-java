	    ...
	    // Keep looping until the high pointer is greater than or equal to the low pointer
		 ...
		 while(low <= high) {
	      ...
	      // If the element at the middle index is equal to the key, return the index
			 ...
			 else if (list.get(mid)<key) {   // If the element at the middle index is less than the key, move the low pointer to the middle + 1
				 
				 low = mid +1;
				
			}
			 else if(list.get(mid)>key) {   // If the element at the middle index is greater than the key, move the high pointer to the middle - 1
				 high = mid-1;
			}
